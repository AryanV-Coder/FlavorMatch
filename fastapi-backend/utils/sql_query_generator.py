...
16. When filtering by is_liked or is_healthy in the WHERE clause, ALWAYS include both fd.is_liked and fd.is_healthy in the SELECT statement to provide complete context
...
- "What does Aryan like to eat?" → SELECT f.family_username AS family_name, m.member_username AS member_name, fd.food, fd.is_liked, fd.is_healthy, fd.timestamp FROM food fd JOIN member m ON fd.member_id = m.member_id JOIN family f ON m.family_id = f.family_id WHERE f.family_username = '{family_name}' AND m.member_username = 'Aryan' AND fd.is_liked = 'Yes' ORDER BY fd.timestamp DESC
...
- "Show healthy foods in my family" → SELECT f.family_username AS family_name, m.member_username AS member_name, fd.food, fd.is_liked, fd.is_healthy, fd.timestamp FROM food fd JOIN member m ON fd.member_id = m.member_id JOIN family f ON m.family_id = f.family_id WHERE f.family_username = '{family_name}' AND fd.is_healthy = 'Healthy' ORDER BY fd.timestamp DESC
